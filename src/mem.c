...
static BYTE _ram[RAM_SIZE];	// RAM_SISE: 1MB
...
} _mem_stat [NUM_PAGES]; // NUM_PAGES: 1024

static pthread_mutex_t mem_lock;

void init_mem(void) {
	memset(_mem_stat, 0, sizeof(*_mem_stat) * NUM_PAGES);	// Set value 0 to _mem_stat with 12288 bytes
	memset(_ram, 0, sizeof(BYTE) * RAM_SIZE);				// Set value 0 to _ram with 1 MB
	pthread_mutex_init(&mem_lock, NULL);
}

/* get 10-low-bit offset of the virtual address */
static addr_t get_offset(addr_t addr) {
	return addr & ~((~0U) << OFFSET_LEN);
}

/* get the 17-bit-high first layer index */
static addr_t get_first_lv(addr_t addr) {
	return addr >> (OFFSET_LEN + PAGE_LEN);
}

/* get the 5-bit middle second layer index */
...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	...
	uint32_t num_pages = (size % PAGE_SIZE) ? size / PAGE_SIZE : size / PAGE_SIZE + 1; // PAGE_SIZE: 1024 - Number of pages we will use
